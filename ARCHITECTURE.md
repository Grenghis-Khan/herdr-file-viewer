...
| `git` | Read-only git queries: status, baseline selection, changed-set, per-file diff. Owns the hardened command builder every git caller shares, and (with `history`) shells out only with read-only subcommands. |
| `history` | Read-only commit-graph queries for the source-control section: `git log --graph` pages with SHAs embedded in the format (git draws the DAG; the module only parses rows), a commit's changed-file set, and the whole-commit patch — all through `git`'s hardened builder, all first-parent for merges. |
...
| `controller` | Orchestrate intents → state changes; hold ephemeral session state, including the root-bound `AnnotationStore`; dispatch renders to the worker; map mouse events against fed-back geometry; and rebuild root-bound services on a worktree switch. Its owned annotation projection root-joins file targets, follows the applied `content_path` rather than the live cursor, and exposes merged line ranges only when the applied render carries a source map. Feature submodules are `mod`, `mouse`, `help`, `finder`, `picker`, `infile`, `lineselect`, `annotation`, `git_apply`, and `graph` (the source-control section: graph paging/selection, commit mode — the unified commit view + the commit-scoped tree — and the focus-gated graph keys). One `Modal` enum type-enforces exclusive input ownership. Only a successful root change clears annotations; failure and same-root paths preserve them. Quitting or switching worktree with a non-empty store raises the `Modal::DiscardConfirm` layer (for a quit, outside search/unzoom) rather than silently discarding it; its `y` proceeds only on a successful clipboard write, and a switch re-validates its held target before committing. |
